...
<validationResult title="{$labels/msg/titleCR}" version="1.2.2, CENIA 2014">
		 	...
		 	<xsl:if test="gmd:identificationInfo/*/gmd:descriptiveKeywords/gmd:MD_Keywords[contains(gmd:thesaurusName/*/gmd:title/gco:CharacterString,'GEMET - INSPIRE themes')]/gmd:keyword/gco:CharacterString">
				...
				<xsl:for-each select="gmd:identificationInfo/*/gmd:descriptiveKeywords/gmd:MD_Keywords[contains(gmd:thesaurusName/*/gmd:title/gco:CharacterString,'GEMET - INSPIRE themes')]/gmd:keyword">
					...
					<test code="3.1">
						...
						<xsl:choose>
							<!-- <xsl:when test="$codelists1/inspireKeywords/value[@name=$kw]!=''"> -->
...
<xsl:if test="not($srv)">
    <test code="CZ-10" level="n"> <!-- TODO - jeste vylepsit -->
    	...
    	<xpath>dataQualityInfo/*/report/DQ_CompletenessOmission[measureIdentification/*/code/*='CZ-COVERAGE']/result</xpath>
		<xsl:variable name="k" select="gmd:dataQualityInfo/*/gmd:report/gmd:DQ_CompletenessOmission[gmd:measureIdentification/*/gmd:code/*='CZ-COVERAGE']"/>
