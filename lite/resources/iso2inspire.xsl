...
<fieldset>
	...
	<xsl:choose>
		<xsl:when test="$serv">
			<input type="hidden" name="hierarchyLevel" value="service"/>
		</xsl:when>
		<xsl:otherwise>
			<xsl:call-template name="drawInput">
 ...
 <div class="cl"> </div>
    	<xsl:choose>
    	  <xsl:when test="$serv">
      			...
      			<xsl:call-template name="drawInput">
      			    ...
      			    <xsl:with-param name="values" select="gmd:identificationInfo/*/gmd:descriptiveKeywords[contains(*/gmd:thesaurusName/*/gmd:title/gco:CharacterString,'ISO 19119')]/*/gmd:keyword/gco:CharacterString"/>
	      		 ...
	      		 <!-- ostatani KW-->
	          	<xsl:for-each select="gmd:identificationInfo/*/gmd:descriptiveKeywords/*[not(contains(gmd:thesaurusName/*/gmd:title/gco:CharacterString,'ISO 19119'))]">
    	  ...
    	  <xsl:otherwise>
    	  	<!--  xsl:variable name="kwins">
      		 ...
      		 <xsl:call-template name="drawInput">
      		  	<xsl:with-param name="name" select="'inspire'"/>
      		    <xsl:with-param name="values" select="gmd:identificationInfo/*/gmd:descriptiveKeywords[substring(*/gmd:thesaurusName/*/gmd:title/gco:CharacterString,1,15) = 'GEMET - INSPIRE']/*/gmd:keyword"/>
	...
	<input type="hidden" id="gemetDate" name="gemetDate" value="{gmd:identificationInfo/*/gmd:descriptiveKeywords/gmd:MD_Keywords[substring(gmd:thesaurusName/*/gmd:title/gco:CharacterString,1,15) = 'GEMET - Concept']/gmd:thesaurusName/*/gmd:date/*/gmd:date}"/>
	  ...
	  <xsl:call-template name="drawInput">
	    ...
	    <xsl:with-param name="values" select="gmd:identificationInfo/*/gmd:resourceConstraints/*/gmd:useLimitation"/>
	    <xsl:with-param name="class" select="'mandatory'"/> 
	    <xsl:with-param name="type" select="'textarea'"/>
	  ...
	  <xsl:call-template name="drawInput">
	    ...
	    <xsl:with-param name="values" select="gmd:identificationInfo/*/gmd:resourceConstraints/*/gmd:otherConstraints"/>
	    <xsl:with-param name="class" select="'mandatory'"/> 
	    <xsl:with-param name="type" select="'textarea'"/>
...
<fieldset>
	...
	<xsl:for-each select="/.|gmd:dataQualityInfo/*/gmd:report/gmd:DQ_DomainConsistency/gmd:result[contains(*/gmd:specification/*/gmd:title/gco:CharacterString,'INSPIRE') or contains(*/gmd:specification/*/gmd:title/gco:CharacterString,'COMMISSION REGULATION')]">
		<xsl:if test="string-length(*/gmd:specification)>0 or(string-length(*/gmd:specification)=0 and position()=1)">
			<div class="cl" id="specification_{position()-1}_">
				<div style="xfloat:left; xwidth:200px">
					<!-- <xsl:call-template name="drawInput">
			    ...
			    <div style="xfloat:left; xwidth:620px">
				 	<xsl:choose>
						<xsl:when test="$serv">
							<xsl:call-template name="drawInput">
