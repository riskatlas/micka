...
<xsl:if test="not($serv)">
	...
	<xsl:call-template name="drawInput">
	    ...
	    <xsl:with-param name="multi" select="2"/>
	    <xsl:with-param name="valid" select="'CZ-3'"/>
	</xsl:call-template>
	...
	<xsl:call-template name="drawInput">
	    ...
	    <xsl:with-param name="multi" select="2"/>
	    <xsl:with-param name="class" select="'mandatory'"/>
	    <xsl:with-param name="valid" select="'CZ-5'"/>
	</xsl:call-template>
	...
	<xsl:call-template name="drawInput">
	    ...
	    <xsl:with-param name="values" select="gmd:identificationInfo/*/gmd:resourceMaintenance/*/gmd:maintenanceAndUpdateFrequency/*/@codeListValue"/>
	...
	<xsl:call-template name="drawInput">
	    ...
	    <xsl:with-param name="values" select="gmd:identificationInfo/*/gmd:resourceMaintenance/*/gmd:userDefinedMaintenanceFrequency"/>
	    <xsl:with-param name="multi" select="2"/>
...
<fieldset>
	...
	<xsl:for-each select="/.|gmd:distributionInfo/*/gmd:distributionFormat">
		<xsl:if test="*/gmd:name/*!='' or(string-length(*/gmd:name/*)=0 and position()=1)">
		...
		<xsl:if test="string-length(*/gmd:specification)>0 or(string-length(*/gmd:specification)=0 and position()=1)">
			<div class="cl" id="specification_{position()-1}_">
			    ...
			    <div style="xfloat:left; xwidth:620px">
				 	<xsl:choose>
						<xsl:when test="$serv">
				            <xsl:variable name="spec1" select="$codeLists/inspireKeywords/value[contains($spec, @code)]"/>
							<xsl:call-template name="drawInput">
						...
						<xsl:otherwise>
				            <xsl:variable name="spec1" select="$codeLists/inspireKeywords/value[contains($spec,@code)]/@code"/>
							<xsl:call-template name="drawInput">
								<xsl:with-param name="name" select="'specification'"/>
							  	<xsl:with-param name="path" select="concat('specification_',position()-1,'_title')"/>
