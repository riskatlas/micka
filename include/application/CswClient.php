...
class CswClient{ 
  ...
  function getDataByGet($url, $name, $id, $template){	
    $url = $url."?service=CSW&version=2.0.2&request=GetRecordById&ID=".$id."&TypeNames=gmd:MD_Metadata&OutputSchema=http://www.isotc211.org/2005/gmd";
	return $this->getDataByURL($url, $name, $template);
  }

  function getDataByURL($url, $name, $template){
