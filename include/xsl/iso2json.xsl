...
<xsl:template match="gmd:MD_Metadata">
    ...
    <xsl:choose>
		...
		$rec['link'] = '<xsl:value-of select="normalize-space(gmd:distributionInfo/*/gmd:transferOptions/*/gmd:onLine/*/gmd:linkage)"/>';
		$rec['links'] = array();
		<xsl:for-each select="gmd:distributionInfo/*/gmd:transferOptions/*/gmd:onLine/*/gmd:linkage">
			$rec['links'][] = '<xsl:value-of select="normalize-space(.)"/>';
		</xsl:for-each>
		...
		$rec['contact'] = '<xsl:value-of select="gmd:contact/*/gmd:organisationName/gco:CharacterString"/>';
    ...
    <xsl:variable name="txt" select="$el/gmd:PT_FreeText/*/gmd:LocalisedCharacterString[@locale=concat('#locale-',$lang)]"/>	
     <xsl:choose>
    	<xsl:when test="string-length($txt)>0">
    	  <xsl:value-of select="translate(substring(normalize-space($txt),0,400), $apos, '')"/>
    	</xsl:when>
    	<xsl:otherwise>
    	  <xsl:value-of select="translate(substring(normalize-space($el/gco:CharacterString),0,400), $apos, '')"/>
    	</xsl:otherwise>
    </xsl:choose>
  </xsl:template>
