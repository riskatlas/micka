    ...
    <MD_Distribution>
  	...
  	<MD_ReferenceSystem>
  	  ...
  	  <RS_Identifier>
  		  ...
  		  <codeSpace>urn:ogc:def:crs:<xsl:value-of select="translate(substring-before(.,':'),$lower,$upper)"/></codeSpace>
  		</RS_Identifier>
  	  </referenceSystemIdentifier>
  	</MD_ReferenceSystem>  
  	</referenceSystemInfo>	 
...
<!-- Verze 1.3 -->  
  ...
  <xsl:variable name="degree" select="//inspire_vs:ExtendedCapabilities/inspire_common:Degree"/>
  <results>
    <MD_Metadata>
    <contact>
      <CI_ResponsibleParty>
		    	...
		    	<keyword><xsl:value-of select="inspire_common:KeywordValue"/></keyword>
		     	<thesaurusName>
				  <CI_Citation>
					...
					<date>
					  <CI_Date>
						<date><xsl:value-of select="inspire_common:OriginatingControlledVocabulary/inspire_common:DateOfPublication"/></date>
						<dateType>
		  				  <CI_DateTypeCode>publication</CI_DateTypeCode> 
		  				</dateType>
		  			  </CI_Date>
    ...
    <xsl:if test="//inspire_common:MetadataDate!=''">
