...
<xsl:stylesheet version="1.0" 
  ...
  <xsl:template match="csw:GetRecords">
	  ...
	  <xsl:for-each select="./*">$params['REQTYPE'] = "<xsl:value-of select="name()" />"; </xsl:for-each>  
	  ...
	  $params['SORTBY'] = "<xsl:value-of select="//ogc:SortBy/ogc:SortProperty/ogc:PropertyName" />|<xsl:value-of select="//ogc:SortBy/ogc:SortProperty/ogc:SortOrder" />";
  </xsl:template>
 ...
 <!-- TRANSACTION -->
  ...
  <xsl:template name="rek">
    ...
    <xsl:variable name="n" select="./ogc:PropertyName"/><xsl:choose>
	  ...
	  <!-- and/or -->
      <xsl:when test="name()='ogc:Or' or name()='ogc:And'">array(
        <xsl:variable name="OrAnd" select="name()"/> 
        <xsl:for-each select="$nod">
          <xsl:call-template name="rek">
            <xsl:with-param name="nod" select="*"/>
		  </xsl:call-template>
          <xsl:if test="not(position()=last())">
            ,"<xsl:value-of select="substring-after($OrAnd,':')"/>",
          </xsl:if>
        </xsl:for-each>)
      </xsl:when>
	  
      <xsl:when test="name()='ogc:Not'">array("Not", <xsl:for-each select="$nod">
      		<xsl:call-template name="rek">
      			<xsl:with-param name="nod" select="*"/>
      		</xsl:call-template>
      	</xsl:for-each>)
      ...
      <xsl:when test="name()='ogc:PropertyIsEqualTo'"><xsl:call-template name="elm"><xsl:with-param name="name" select="$n"/></xsl:call-template> = '<xsl:value-of select="./ogc:Literal"/>'"</xsl:when>
      <xsl:when test="name()='ogc:PropertyIsNotEqualTo'"><xsl:call-template name="elm"><xsl:with-param name="name" select="$n"/></xsl:call-template> != '<xsl:value-of select="./ogc:Literal"/>'"</xsl:when>
      <xsl:when test="name()='ogc:PropertyIsLike'"><xsl:call-template name="elm"><xsl:with-param name="name" select="$n"/></xsl:call-template> LIKE '<xsl:value-of select="translate(./ogc:Literal,@wildCard,'%')"/>'"</xsl:when>
      <xsl:when test="name()='ogc:PropertyIsLessThan'"><xsl:call-template name="elm"><xsl:with-param name="name" select="$n"/></xsl:call-template> &lt; '<xsl:value-of select="./ogc:Literal"/>'"</xsl:when>
      <xsl:when test="name()='ogc:PropertyIsGreaterThan'"><xsl:call-template name="elm"><xsl:with-param name="name" select="$n"/></xsl:call-template> &gt; '<xsl:value-of select="./ogc:Literal"/>'"</xsl:when>
      <xsl:when test="name()='ogc:PropertyIsLessThanOrEqualTo'"><xsl:call-template name="elm"><xsl:with-param name="name" select="$n"/></xsl:call-template> &lt;= '<xsl:value-of select="./ogc:Literal"/>'"</xsl:when>
      <xsl:when test="name()='ogc:PropertyIsGreaterThanOrEqualTo'"><xsl:call-template name="elm"><xsl:with-param name="name" select="$n"/></xsl:call-template> &gt;= '<xsl:value-of select="./ogc:Literal"/>'"</xsl:when>
      <xsl:when test="name()='ogc:Intersects'"><xsl:call-template name="elm"><xsl:with-param name="name" select="$n"/></xsl:call-template>"</xsl:when>
      <xsl:when test="name()='ogc:Within'"><xsl:call-template name="elm"><xsl:with-param name="name" select="$n"/></xsl:call-template>"</xsl:when>
      <xsl:when test="name()='ogc:BBOX'">"_BBOX_='<xsl:value-of select="gml:Envelope/gml:lowerCorner"/><xsl:text> </xsl:text><xsl:value-of select="gml:Envelope/gml:upperCorner"/>'"</xsl:when>
	...
	<xsl:template name="elm">
		...
		<!-- jeste dodat CRS -->
			...
			<!-- opacne -->
			<xsl:if test="name()='ogc:Within'"> 1</xsl:if><xsl:if test="contains(name(*),'Envelope')">1</xsl:if>'</xsl:when>
      	...
      	<!-- additional ISO queryables -->
			...
			<xsl:when test="contains($n,'language')">"@language</xsl:when><!-- OK -->
